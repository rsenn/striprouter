...
Connection::Connection(const ConnectionPoint& _start, const ConnectionPoint& _end) : start(_start), end(_end) {}
...
bool
Circuit::hasParserError() const {
  return parserErrorVec.size() > 0UL;
}

ConnectionViaVec
Circuit::genConnectionViaVec() const {
  ConnectionViaVec v;
  for(auto c : connectionVec) {
    ...
    auto startRelPin = packageToPosMap.at(startComponent.packageName).at(c.start.pinIdx);
...
StartEndVia
Circuit::calcComponentFootprint(std::string componentName) const {
...
PinViaVec
Circuit::calcComponentPins(std::string componentName) const {
  PinViaVec v;
