...
void
CircuitFileWriter::updateComponentPositions(const std::string circuitFilePath, const Circuit& circuit) {
  //{
  ...
  if(!inFile.good()) {
    throw std::runtime_error(fmt::format("Could not open file for read. path=\"{}\"", circuitFilePath));
  }

  auto tmpFilePath = circuitFilePath + ".tmp";
  std::ofstream outFile(tmpFilePath);
  if(!outFile.good()) {
    throw std::runtime_error(fmt::format("Could not open file for write. path=\"{}\"", tmpFilePath));
  }
  ...
  if(result) {
    throw std::runtime_error(fmt::format("Could not delete old file. path=\"{}\"", circuitFilePath));
  }

  result = rename(tmpFilePath.c_str(), circuitFilePath.c_str());
  if(result) {
    throw std::runtime_error(
        fmt::format("Could not replace file. new=\"{}\" old=\"{}\"", tmpFilePath, circuitFilePath));
  }
}

ComponentLine
CircuitFileWriter::parseComponentLine(const std::string& lineStr) {
