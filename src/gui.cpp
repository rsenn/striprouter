...
Pos
boardToScrPos(const Pos& boardPos, const float zoom, const Pos& boardScreenOffset) {
  return boardPos * zoom + boardScreenOffset;
}

Pos
screenToBoardPos(const Pos& scrPos, const float zoom, const Pos& boardScreenOffset) {
  return (scrPos - boardScreenOffset) / zoom;
}

Pos
getMouseScrPos(const IntPos& intMousePos) {
  return intMousePos.cast<float>();
}

Pos
getMouseBoardPos(const IntPos& intMousePos, const float zoom, const Pos& boardScreenOffset) {
  return screenToBoardPos(getMouseScrPos(intMousePos), zoom, boardScreenOffset);
}

std::string
getComponentAtBoardPos(Circuit& circuit, const Pos& boardPos) {
...
void
setComponentPosition(Circuit& circuit, const Via& mouseBoardVia, const std::string& componentName) {
