...
void renderDragStatus(const IntPos mouseScrPos, OglText& dragText);
...
GeneticAlgorithm geneticAlgorithm(N_ORGANISMS_IN_POPULATION, CROSSOVER_RATE, MUTATION_RATE);

// Misc
void resetInputLayout();
nanogui::Button* saveBestLayoutButton;
std::string CIRCUIT_FILE_PATH = get_bindir() + "/circuits/example.circuit";
...
public:
  static const std::string DIAG_FONT_FILE;
  static constexpr int DIAG_FONT_SIZE = 12;
  static constexpr int DRAG_FONT_SIZE = 12;

  Application(Render& r)
      : render(r), DIAG_FONT_PATH(get_bindir() + DIAG_FONT_FILE),
        diagText(get_bindir() + DIAG_FONT_FILE, DIAG_FONT_SIZE),
        dragText(get_bindir() + DIAG_FONT_FILE, DRAG_FONT_SIZE), nanogui::Screen(IntPos(windowW, windowH),
    ...
    form->addGroup("Costs");
    {
      auto w = form->addVariable<int>("Wire",
                                      [&](int v) {
    ...
    {
      auto w = form->addVariable<int>("Strip",
                                      [&](int v) {
                                        auto lock = inputLayout.scopeLock();
    ...
    {
      auto w = form->addVariable<int>("Via",
                                      [&](int v) {
                                        auto lock = inputLayout.scopeLock();
    ...
    {
      auto w = form->addVariable<int>("Cut",
                                      [&](int v) {
                                        auto lock = inputLayout.scopeLock();
    ...
    form->addGroup("Best Layout");
    {
      saveBestLayoutButton = form->addButton("Save to .svg files", [this]() {
        ...
        new nanogui::MessageDialog(this, nanogui::MessageDialog::Type::Information, "Saved .svg files", ss.str());
      });
  ...
  mouseButtonEvent(const IntPos& p, int button, bool down, int modifiers) {
    ...
    if(down) {
      ...
      if(componentName != "") {
        ...
        {
          auto lock = inputLayout.scopeLock();
          auto pin0BoardPos = inputLayout.circuit.componentNameToComponentMap[componentName].pin0AbsPos.cast<float>();
  ...
  virtual bool
  scrollEvent(const IntPos& p, const Eigen::Vector2f& rel) {
  ...
  virtual bool
  resizeEvent(const IntPos& size) {
    windowW = size.x();
    windowH = size.y();
    projMat = glm::ortho(0.0f, static_cast<float>(size.x()), static_cast<float>(size.y()), 0.0f, 0.0f, 100.0f);
  ...
  drawContents() {
    ...
    renderDragStatus(mousePos(), dragText);
    ...
    //    if (!(intervalIdx++ % 20)) {
    //      auto tgaFileName = fmt::format(get_bindir()+ "/video/frame_{:05d}.tga",
    //      frameIdx++);
...
private:
  Render& render;
  std::string DIAG_FONT_PATH;
  OglText dragText;
  OglText diagText;
  GLuint vertexArrayId;
};

const std::string Application::DIAG_FONT_FILE = "/fonts/RobotoMono-Regular.ttf";
void
handleMouseDragOperations(const IntPos& mousePos) {
  ...
  if(isComponentDragActive) {
    ...
    auto component = inputLayout.circuit.componentNameToComponentMap[dragComponentName];
    auto footprint = inputLayout.circuit.calcComponentFootprint(dragComponentName);
...
void
renderDragStatus(const IntPos mouseScrPos, OglText& dragText) {
  if(isComponentDragActive) {
    ...
    dragText.print(projMat, mouseScrPos.x(), mouseScrPos.y(), 0, fmt::format("({},{})", mouseVia.x(), mouseVia.y()));
  }
}
...
centerBoard() {
  ...
  panOffsetScrPos = Pos(windowW / 2.0f - boardScreenW / 2.0f, windowH / 2.0f - boardScreenH / 2.0f);
...
routerThread() {
  while(!threadStopRouter.isStopped()) {
    ...
    if(useRandomSearch) {
      for(int i = 0; i < static_cast<int>(threadLayout.circuit.connectionVec.size()); ++i) {
    ...
    }
    {
      ...
      Router router(threadLayout, connectionIdxVec, threadStopRouter, inputLayout, currentLayout, maxRenderDelay);
    ...
    if(!useRandomSearch) {
      auto lock = geneticAlgorithm.scopeLock();
      geneticAlgorithm.releaseOrdering(orderingIdx, threadLayout.nCompletedRoutes, threadLayout.cost);
    }
    ...
    // Update bestLayout
    {
      auto inputLock = inputLayout.scopeLock();
      auto bestLock = bestLayout.scopeLock();
      auto hasMoreCompletedRoutes = threadLayout.nCompletedRoutes > bestLayout.nCompletedRoutes;
      auto hasEqualRoutesAndBetterScore =
          threadLayout.nCompletedRoutes == bestLayout.nCompletedRoutes && threadLayout.cost < bestLayout.cost;
      auto isBasedOnOtherLayout = !bestLayout.isBasedOn(threadLayout);
      if(hasMoreCompletedRoutes || hasEqualRoutesAndBetterScore || isBasedOnOtherLayout) {
...
resetInputLayout() {
  ...
  {
    auto lock = geneticAlgorithm.scopeLock();
    geneticAlgorithm.reset(static_cast<int>(inputLayout.circuit.connectionVec.size()));
  }
}
...
parseCommandLineArgs(int argc, char** argv) {
  ...
  parser.set_optional<bool>("r", "random", false, "Use random search instead of genetic algorithm");
  parser.set_optional<bool>("e", "exitcomplete", false, "Print stats and exit when first complete layout is found");
  parser.set_optional<long>("a", "exitafter", -1, "Print stats and exit after specified number of checks");
  parser.set_optional<long>("p", "checkpoint", -1, "Print stats at interval");
  parser.set_optional<std::string>("c", "circuit", CIRCUIT_FILE_PATH, "Path to .circuit file");
...
printStats() {
  auto inputLock = inputLayout.scopeLock();
  auto bestLock = bestLayout.scopeLock();
  fmt::print("search={} nChecks={:n} Best: nCompletedRoutes={:n} nFailedRoutes={:n} "
             "cost={:n}\n",
