...
OglText::OglText(const std::string& fontPath, int fontH)
    : oglInitialized_(false), texWH_(0), fontH_(fontH), textProgramId_(0), textBackgroundProgramId(0), face_(0) {
...
void
OglText::print(glm::mat4x4& projMat, int x, int y, int nLine, const std::string& str, bool drawBackground) {
...
int
OglText::calcStringWidth(const std::string& str) {
  int strW = 0;
...
void
OglText::createFreeType(const std::string& fontPath) {
...
OglText::createFontTexture() {
  ...
  if(!fitOk) {
    fmt::print("Error: Unable to fit font into texture. font_h={} "
...
bool
OglText::renderFont(std::vector<unsigned char>& fontVec) {
  ...
  for(int i = 32; i <= 126; ++i) {
    ...
    charMeta_.push_back({tex_x,
                         tex_y,
                         static_cast<int>(glyph_bitmap.width),
                         static_cast<int>(glyph_bitmap.rows),
                         -slot->bitmap_top,
...
void
OglText::drawText(int x, int y, int nLine, const std::string& str) {
...
void
OglText::drawTextBackground(int x, int y, int nLine, const std::string& str) {
