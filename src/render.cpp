...
Render::Render()
    : componentText_(OglText(get_bindir() + CIRCUIT_FONT_FILE, CIRCUIT_FONT_SIZE)),
      notationText_(OglText(get_bindir() + CIRCUIT_FONT_FILE, NOTATION_FONT_SIZE)), fillProgramId_(0), vertexBufId_(0) {
}
  ...
  for(auto routeSectionVec : layout_->routeVec) {
    for(auto section : routeSectionVec) {
      ...
      if(start.x() != end.x() && start.y() == end.y()) {
        ...
        drawThickLine(section.start.via.cast<float>(), section.end.via.cast<float>(), CONNECTION_WIDTH, rgba);
      }
    }
  }
}
...
Render::drawComponents() {
  ...
  for(auto ci : layout_->circuit.componentNameToComponentMap) {
    ...
    bool isPin0 = true;
    int pinIdx = 0;
    for(auto pinVia : const_cast<Layout&>(*layout_).circuit.calcComponentPins(componentName)) {
    ...
    componentText_.print(
        projMat_, txtCenterS.x() - stringWidth / 2.0f, txtCenterS.y() - stringHeight / 2.0f, 0, componentName, true);
  }
}

void
Render::drawStripboardSection(const StartEndVia& viaStartEnd) {
  // Copper strip
...
void
Render::drawRatsNest(bool showOnlyFailedBool) {
...
Render::drawDiag() {
  ...
  if(layout_->diagStartVia.isValid) {
    drawFilledCircle(layout_->diagStartVia.via.cast<float>(), 1.5, RGBA(1, 1, 1, 1));
    printNotation(layout_->diagStartVia.via.cast<float>(), 0, "start");
  }
  if(layout_->diagEndVia.isValid) {
    drawFilledCircle(layout_->diagEndVia.via.cast<float>(), 1.5, RGBA(1, 1, 1, 1));
    printNotation(layout_->diagEndVia.via.cast<float>(), 0, "end");
  }
  // Draw wire jump labels
  for(int y = 0; y < layout_->gridH; ++y) {
    for(int x = 0; x < layout_->gridW; ++x) {
      auto& wireToVia = layout_->diagTraceVec[layout_->idx(Via(x, y))].wireToVia;
  ...
  if(v.isValid) {
    ...
    printNotation(mouseBoardPos_, nLine++, fmt::format("wireCost: {}", viaCost.wireCost));
    printNotation(mouseBoardPos_, nLine++, fmt::format("stripCost: {}", viaCost.stripCost));
    const auto& viaTrace = layout_->diagTraceVec[idx];
    printNotation(mouseBoardPos_, nLine++, fmt::format("wireBlocked: {}", viaTrace.isWireSideBlocked));
    // Nets
    ...
    if(setIdxSize) {
      ...
      printNotation(mouseBoardPos_, nLine++, fmt::format("setSize: {}", layout_->viaSetVec[setIdx].size()));
    }
  }
}
...
void
Render::drawFilledCircle(const Pos& center, float radius, const RGBA& rgba) {
...
void
Render::addFilledCircle(const Pos& center, float radius) {
  if(isPointOutsideScreen(center)) {
    return;
  }
  radius *= zoom_;
  ...
  if(centerS.x() + radius < 0.0f || centerS.x() - radius > windowW_ || centerS.y() + radius < 0.0f ||
     centerS.y() - radius > windowH_) {
    return;
  }
...
void
Render::drawFilledCircleBuffer(const RGBA& rgba) {
  if(!triVec.size()) {
    return;
  }
  setColor(rgba);
...
void
Render::drawThickLine(const Pos& start, const Pos& end, float radius, const RGBA& rgba) {
  if(isLineOutsideScreen(start, end)) {
    return;
  }
  radius *= zoom_;
...
bool
Render::isPointOutsideScreen(const Pos& p) {
  float pad = 50.0f;
  return p.x() < -pad || p.x() >= windowW_ + pad || p.y() < -pad || p.y() >= windowH_ + pad;
}

bool
Render::isLineOutsideScreen(const Pos& start, const Pos& end) {
  float pad = 50.0f;
  return (start.x() < -pad && end.x() < -pad) || (start.x() >= windowW_ + pad && end.x() >= windowW_ + pad) ||
         (start.y() < -pad && end.y() < -pad) || (start.y() >= windowW_ + pad && end.y() >= windowH_ + pad);
}

float
Render::setAlpha(const Via& v) {
  auto mouseNet = getMouseNet();
  if(!mouseNet.size()) {
    return 1.0f;
  }
...
ValidVia
Render::getMouseVia() {
  Via v = Via(static_cast<int>(mouseBoardPos_.x() + 0.5f), static_cast<int>(mouseBoardPos_.y() + 0.5f));
  if(v.x() >= 0 && v.y() >= 0 && v.x() < layout_->gridW && v.y() < layout_->gridH) {
