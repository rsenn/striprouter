...
SvgPathVec
SvgWriter::writeFiles(const std::string circuitFilePath) {
  auto baseName = circuitFilePath.substr(0, circuitFilePath.find_last_of("."));
  auto wireSvgPath = fmt::format(
      "{}.{}.{}.{}.{}.svg", baseName, layout_.nCompletedRoutes, layout_.nFailedRoutes, layout_.cost, "wires");
  auto stripCutSvgPath = fmt::format(
      "{}.{}.{}.{}.{}.svg", baseName, layout_.nCompletedRoutes, layout_.nFailedRoutes, layout_.cost, "cuts");
...
svg::Document
SvgWriter::initDoc(const std::string& svgPath, const bool drawMirrorImage) {
...
void
SvgWriter::writeWireSvg(const std::string wireSvgPath) {
...
void
SvgWriter::writeStripCutSvg(const std::string cutSvgPath) {
...
void
SvgWriter::drawBackground(svg::Document& doc) {
...
void
SvgWriter::drawBoardOutline(svg::Document& doc) {
  svg::Polygon border(svg::Fill(WHITE), svg::Stroke(BOARD_OUTLINE, BLACK));
  border << svg::Point(-1.0, -1.0) << svg::Point(layout_.gridW, -1.0) << svg::Point(layout_.gridW, layout_.gridH)
         << svg::Point(-1.0, layout_.gridH);
  doc << border;
}

void
SvgWriter::drawCorners(svg::Document& doc) {
  doc << svg::Circle(svg::Point(0, 0), CORNER_VIA_RADIUS, svg::Fill(BLACK), svg::Stroke(CORNER_VIA_RADIUS, BLACK));
  doc << svg::Circle(svg::Point(layout_.gridW - 1, 0),
                     CORNER_VIA_RADIUS,
                     svg::Fill(BLACK),
  ...
  doc << svg::Circle(svg::Point(layout_.gridW - 1, layout_.gridH - 1),
                     ...
                     svg::Fill(BLACK),
                     svg::Stroke(CORNER_VIA_RADIUS, BLACK));
  doc << svg::Circle(svg::Point(0, layout_.gridH - 1),
                     CORNER_VIA_RADIUS,
                     svg::Fill(BLACK),
                     svg::Stroke(CORNER_VIA_RADIUS, BLACK));
}
...
void
SvgWriter::drawVias(svg::Document& doc) {
  for(int y = 0; y < layout_.gridH; ++y) {
    for(int x = 0; x < layout_.gridW; ++x) {
      doc << svg::Circle(svg::Point(x, y), VIA_RADIUS, svg::Fill(BLACK), svg::Stroke(VIA_RADIUS, BLACK));
    }
  }
}

void
SvgWriter::drawWireSections(svg::Document& doc) {
  for(auto routeSectionVec : layout_.routeVec) {
    for(auto section : routeSectionVec) {
      ...
      if(start.x() != end.x() && start.y() == end.y()) {
        doc << svg::Line(svg::Point(section.start.via.x(), section.start.via.y()),
...
void
SvgWriter::drawStripCuts(svg::Document& doc) {
  for(auto& v : layout_.stripCutVec) {
    ...
    doc << svg::Rectangle(start, CUT_W, CUT_H, svg::Fill(BLACK), svg::Stroke(VIA_RADIUS, BLACK));
  }
}

void
SvgWriter::drawTitle(svg::Document& doc, const std::string& titleStr) {
  doc << svg::Text(svg::Point(0.0, -2.0), titleStr, BLACK, svg::Font(2.0, "Arial"));
}

void
SvgWriter::drawTitleMirror(svg::Document& doc, const std::string& titleStr) {
